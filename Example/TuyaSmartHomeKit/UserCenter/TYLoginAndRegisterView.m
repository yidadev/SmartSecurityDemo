...
- (void)initView {
    ...
    switch (currentType) {
        ...
        case TYLoginAndRegisterViewTypeRegister:
        {
            ...
            break;
        case TYLoginAndRegisterViewTypeUid:
        {
            accountKeyLabel.text = @"Customer ID";
            self.accountField.placeholder = @"Customer Unique ID";
            
            UIButton *pwdLoginButton = [UIButton buttonWithType:UIButtonTypeCustom];
            [pwdLoginButton setTitle:@"Login or Register" forState:UIControlStateNormal];
            pwdLoginButton.frame = CGRectMake(20, CGRectGetMaxY(self.passwordField.frame) + 20, APP_SCREEN_WIDTH - 40, 44);
            pwdLoginButton.layer.cornerRadius = 5;
            pwdLoginButton.backgroundColor = UIColor.orangeColor;
            [pwdLoginButton addTarget:self action:@selector(uidLoginOrRegisterAction) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:pwdLoginButton];
...
- (void)uidLoginOrRegisterAction {
    if (self.delegate && [self.delegate respondsToSelector:@selector(loginAndRegisterViewTrigerredAction:)]) {
        [self.delegate loginAndRegisterViewTrigerredAction:TYLoginAndRegisterViewActionTypeUid];
    }
}
